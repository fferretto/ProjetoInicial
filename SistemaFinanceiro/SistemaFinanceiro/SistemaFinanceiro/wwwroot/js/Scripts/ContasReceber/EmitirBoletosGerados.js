...
function EnviarEmailSelecionados() {
    var table = $('#gridStatusEnvioEmail').DataTable();

    var meuData = '';
    var trs = $(table.cells().nodes()).parent();
    trs.each(function (index, value) {

        var codEmissaoBoleto = $(value).find('.codEmissaoBoleto')[0].innerHTML;  


        $(value).find('.StatusRet')[0].innerHTML = "Enviando...";

        var data = "codEmissaoBoleto=" + codEmissaoBoleto;
        ...
        $.ajax({
            ...
            success: function (data) {
                $.unblockUI();
                if (data.success) {
                    $(value).find('.StatusRet')[0].innerHTML = "Enviado";
                    $(value).find('.MensagemRet')[0].innerHTML = data.responseText;

                }
                else {
                    $(value).find('.StatusRet')[0].innerHTML = "Falha";
                    $(value).find('.MensagemRet')[0].innerHTML = data.responseText;
                }
            },
            error: function (data) {
