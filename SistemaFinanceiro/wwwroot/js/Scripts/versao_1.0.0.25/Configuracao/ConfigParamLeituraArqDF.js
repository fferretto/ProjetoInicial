function carregaPagina() {
    ...
    if ($("#codigoArquivoConciliacao").val() != 0) {
        $("#TipoArquivo").val('REM');
        $("#btnRem").click();
        $("#liRem").addClass('active');
    }
    else {
        if ($("#TipoArquivo").val() == 'REM') {
            $("#TipoArquivo").val('REM');
            $("#btnRem").click();
            $("#liRem").addClass('active');
        }
...
function CarregaCliente(codigoCli) {
    ...
    if (codigoCli != "") {       
        ...
        $.ajax({
            type: 'Post',
            url: montaUrl('/Configuracao/ConfigParamLeituraArqDF/BuscaCliente/?codigoCliente=' + codigoCli + '&tipoArquivo=' + $("#TipoArquivo").val()),
            dataType: 'json',
            success: function (data) {
                ...
                if (data.codigoArquivoConciliacao != null && data.codigoArquivoConciliacao > 0) {

                    document.getElementById("codigoFormaVerificacaoArq").selectedIndex = (data.codigoFormaVerificacao - 1)
                    if (data.tipoArquivo == 'REM') {
                        document.getElementById("extensaoArquivo").value = data.extensaoArquivoREM.trim();
                    }
                    else {
                    ...
                    $("#TipoArquivo").val(data.tipoArquivo);                  
                    ...
                    $("#codigoArquivoConciliacao").val(data.codigoArquivoConciliacao);
...
function SalvarConfiguracao() {
    ...
    var data = "codigoCliente=" + $("#codigoCliente").val()
        ...
        + "&codigoArquivoConciliacao=" + $("#codigoArquivoConciliacao").val()
