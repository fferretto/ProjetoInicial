...
$(".codContaCorrente").change(function () {
    ...
    var codconta = $('.codContaCorrente option:selected').val();

    var data = "codContaCorrente=" + codconta

    var _url = montaUrl("/Generico/CadastrosDiversos/GetBancoByContaCorrente")
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success) {
                var nmBanco = data.nmBanco;
                AltercaoContaCorrente(data.codBanco);
            }
            else {
        ...
        error: function (data) {
            msgErro('Ocorreu uma falha inesperada. Favor contactar o suporte técnico.');
        }
    });
